...
@section('content')
    ...
    <section>
        <div class="container mt-2">
            <div class=" m-2 checkRadioContainer justify-content-around text-center" >
                @foreach($sellTypes as $sell)
                    @if($sell['id'] == $sell_type_id)
                            <a  class="col-6 col-lg-2 m-4 m-lg-3"  style="background:#17286E;color:white;cursor: pointer;" >
                    ...
                    @else
                        <a id="radio11" class="col-6 col-lg-2 m-3 m-lg-3" style="color:black;border:1px solid #17286E;cursor: pointer" href="{{route('type.subcategory',['type_category'=>$sell['id']])}}">
                                <span>{{$sell['Title_'. App::getlocale()]}}</span>
                        </a>
                    @endif
                @endforeach
            ...
            <form action="{{route('subcategory-search-products')}}" method="post">
                ...
                <div class="row mt-4">
                    ...
                    <div class="form-group col-12 col-lg-4">
                        <label for="exampleInputEmail1" >{{__('category.payment_method')}}</label>
                        <select class="form-control catDrop" name="payment_method">
                            <option value="cash">{{__('category.cash')}}</option>
                            <option value="installment">{{__('category.installments')}}</option>
                        </select>
                    </div>
                    <div class="form-group col-12 col-lg-4">
                        <label for="exampleInputEmail1" >{{__('category.Country')}}</label>
                        <select class="form-control catDrop" id="city_id" name="city_id" >
                        <option value="sad">{{__('category.selectCountry')}}</option>
                    ...
                    <div class="form-group col-12 col-lg-4">
                        <label for="exampleInputEmail1">{{__('category.City')}}</label>
                    ...
                    <div class="form-group col-12 col-lg-4">
                        <label for="exampleInputEmail1" >{{__('category.Price_Range')}}</label>
                        <div class="row">
                            <div class="col-6">
                                <input type="text" class="form-control catDrop" placeholder="{{__('category.From')}}" name="priceFrom">
                            </div>
                            <div class="col-6">
                                <input type="text" class="form-control catDrop" placeholder="{{__('category.To')}}" name="priceTo">
                            </div>
            ...
            {{--
                                <div class="form-group col-4">
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <input type="text" class="form-control catDrop" placeholder="{{__('category.From')}}" name="year_from">
                                        </div>
                                        <div class="col-6">
                                            <input type="text" class="form-control catDrop" placeholder="{{__('category.To')}}"  name="year_to">
                                        </div>
                                    </div>
                                </div>
            --}}
            ...
            <button type="submit" class="btn w-100 mt-5 backgroundColor" style=" color: #fff;">{{__('category.applayChanges')}}</button><br><br>
