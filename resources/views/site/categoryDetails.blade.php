            ...
            <div class="checkRadioContainer d-flex justify-content-around text-center" >
                @foreach($sellTypes as $sell)
                    @if($sell['id'] == $sell_type_id)
                            <a  class="col-2 m-1" style="background:#17286E;color:white;cursor: pointer" >
                    ...
                    @else
                        <a id="radio11" class="col-2 m-1" style="color:black;border:1px solid #17286E;cursor: pointer" href="{{route('type.subcategory',['type_category'=>$sell['id']])}}">
                                <span>{{$sell['Title_'. App::getlocale()]}}</span>
                        </a>
                    @endif
                @endforeach
