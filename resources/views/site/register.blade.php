        ...
        <form class="signform " id="signupform"  method="post" action="{{route('submit.register',['type'=>$type])}}" enctype="multipart/form-data">
            ...
            <div id="form1">
                ...
                <div class="form-group form-check">
                    ...
                    <label class="form-check-label" for="exampleCheck1">{{__('dina.Do_you_agree')}},<a href="text-decoration-none" style="color:#17286E"> Click here to Show</a></label>
                </div>
                {{-- <a type="submit" class="btn btn-primary w-100 font-weight-bold" href="{{route('register.subscription')}}">Next</a><br><br> --}}
                <div type="submit" class="btn btn-primary w-100 font-weight-bold" style="background:#17286E" onclick="changSignup()">
                    {{__('dina.next')}}</div><br><br>
                <a class=" font-weight-bold" style="color:#17286E" href="{{route('auth.login')}}">{{__('dina.Already_have_an_account')}}? Login here</a><br><br>
                @if($type == 'maintenance')
                    <a class=" font-weight-bold" style="color:#17286E" href="{{route('register',['type'=>'vendor'])}}">Are you vendor?</a><br><br>
                @else
                    <a class=" font-weight-bold" style="color:#17286E" href="{{route('register',['type'=>'maintenance'])}}">{{__('dina.Do_you_have_a_maintenance_center')}} ? Create your store</a><br><br>
                @endif
            ...
            <div id="form2" style="display: none; !important">
                <h5 class="text-center font-weight-bold mb-4">
                   {{__('dina.Account_Subscription')}}
                </h5>
                <div class="form-group">
                    <label for="input11"  class="font-weight-bold">{{__('dina.Company’s_Name')}}</label>
                    <input type="text" class="form-control inputform" id="input11" aria-describedby="emailHelp" name="company_name">
                </div>
                @if($type != 'maintenance')

                    <div class="form-group">
                        <label for="input12" class="font-weight-bold">{{__('dina.Shop_type')}}</label>
            ...
            <div id="form4" style="display: none">
                <h5 class="text-center font-weight-bold mb-4">
                   {{__('dina.Social_Media')}}
                </h5>
                <div class="form-group">
                    <label for="input31">{{__('dina.Profile_Image')}}</label>
                    <input type="file" class="form-control"   name="photo">
                </div>
                <div class="form-group">
                    <label for="input31">{{__('dina.Cover_Image')}}</label>
                    <input type="file" class="form-control " name="cover">
                </div>
                <div class="form-group">
                    <label for="input31">{{__('dina.Facebook')}}</label>
                    <input type="text" class="form-control inputform" id="input31" aria-describedby="emailHelp" name="facebook">
                </div>
                <div class="form-group">
                    <label for="input32">{{__('dina.Twitter')}}</label>
                    <input type="text" class="form-control inputform" id="input32" name="twitter">
                </div>
                <div class="form-group">
                    <label for="input33">{{__('dina.Instagram')}}</label>
                    <input type="text" class="form-control inputform" id="input33" name="instgram">
                </div>

                @if($type == 'maintenance')
                    <div class="btn btn-primary w-100 font-weight-bold" style="background:#17286E" onclick="changSignup4()">{{__('dina.next')}}</div><br><br>
                @else
                    <input class="btn btn-primary w-100 font-weight-bold" id="angry" type="submit" style="display: none;background:#17286E" value="{{__('dina.done')}}">
                @endif
            </div>
